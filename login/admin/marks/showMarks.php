...
<p id="para">Table showing the MARKS obtained by all the students in the college
...
<?php
 ...
 if(isset($_SESSION['semNo'])){
    $sem=$_COOKIE["sem"];
    $semNo=$_SESSION['semNo'];
}elseif(isset($_COOKIE['sem1'])){

    $sem=$_COOKIE["sem1"];
    $semNo=$_COOKIE["semNo1"];
...
if(isset($_GET['search_value'])){
    ...
    $query="SELECT branch,student_id,`$sub1` as $one1,`$sub2` as $two1,`$sub3` as $three1,`$sub4` as $four1,`$sub5` as $five1,`$sub6` as $six1,total FROM `$semNo` 
    WHERE CONCAT( `student_id`, `branch`,`$sub1`, `$sub2`, `$sub3`, `$sub4`, `$sub5`, `$sub6`, `total`)LIKE '%$search_value%' order by $order $sort";

    $result=mysqli_query($conn1,$query);
    }
    else{
        $query="SELECT branch,student_id,`$sub1` as $one1,`$sub2` as $two1,`$sub3` as $three1,`$sub4` as $four1,`$sub5` as $five1,`$sub6` as $six1,total  FROM `$semNo` ORDER BY $order $sort";
        $result=mysqli_query($conn1,$query);
    }
...
<table >
        <tr> 
            ...
            <th>Total<a href='?order=total&sort=$sort'><i class='fas fa-caret-down'></i></a></th>
            <th>Average</th>
            <th>Grade</th>
            <th>Result</th>
...
while ($row = mysqli_fetch_array($result)) {
    ...
    if($one>=10 && $two>=10&&$three>=10&&$four>=10 && $five>=10 && $six>=10)
    {
        ...
        $pass="<strong style=color:green ; > PASSED !!  </strong>";
    }else{
        $grade="F";
    ...
    <td>
    <a href='../modify/updateMarks.php?student_id=$row[student_id]&semNo=$semNo&sem=$sem&one=$one&two=$two&three=$three&four=$four&five=$five&six=$six'>
    <input type='submit' value='Edit' style='background-color:green;'></input>  </a>
    </td>

    <td><a href='../modify/deleteMarks.php?student_id=$row[student_id]&semNo=$semNo' onclick ='return checkdelete() '> 
...
if(isset($_GET['search_value'])){
$search_value=$_GET['search_value'];
$query2="SELECT * FROM `$semNo` WHERE CONCAT(`student_id`, `branch`, `$one`, `$two`, `$three`, `$four`, `$five`, `$six`, `total`) LIKE '%$search_value%' order by $order $sort ";
$conn2=mysqli_connect("localhost","root","","marks");
...
<table >
        <tr> 
            ...
            <th>Student Id<a href='?sem=$sem&semNo=$semNo&order=student_id&sort=$sort'><i class='fas fa-caret-down'></i></a></th>
            <th>branch<a href='?order=branch&sort=$sort'><i class='fas fa-caret-down'></i></a></th>
...
<button style="text-align: center; margin-left:0; margin-right:0 ;color:white;"><a href="./marks.html">Register</a></button>
</body>
</html>